...
func DefaultAudienceMatchingStrategy(haystack []string, needle []string) error {
	...
	for _, n := range needle {
		nu, err := url.Parse(n)
		if err != nil {
			return errorsx.WithStack(ErrInvalidRequest.WithHintID(i18n.ErrHintMalformedRequestAudience, n).WithWrap(err).WithDebug(err.Error()))
		}
		...
		for _, h := range haystack {
			hu, err := url.Parse(h)
			if err != nil {
				return errorsx.WithStack(ErrInvalidRequest.WithHintID(i18n.ErrHintMalformedWhitelistAudience, h).WithWrap(err).WithDebug(err.Error()))
			}
		...
		if !found {
			return errorsx.WithStack(ErrInvalidRequest.WithHintID(i18n.ErrHintAudienceNotAllowed, n))
		}
	}
