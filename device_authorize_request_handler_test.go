...
func TestNewDeviceAuthorizeGetRequest(t *testing.T) {
	...
	for k, c := range []struct {
		...
		t.Run(fmt.Sprintf("case=%d", k), func(t *testing.T) {
			...
			c.conf.Store = store
			ar, err := c.conf.NewDeviceAuthorizeGetRequest(context.Background(), c.r)
...
func TestNewDeviceAuthorizePostRequest(t *testing.T) {
	var store *MockStorage
	for k, c := range []struct {
		desc          string
		conf          *Fosite
		...
		t.Run(fmt.Sprintf("case=%d", k), func(t *testing.T) {
			...
			c.conf.Store = store
			ar, err := c.conf.NewDeviceAuthorizePostRequest(context.Background(), c.r)
			if c.expectedError != nil {
				assert.EqualError(t, err, c.expectedError.Error())
			} else {
				require.NoError(t, err)
