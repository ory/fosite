...
func matchScopes(ss fosite.ScopeStrategy, granted, scopes []string) error {
	for _, scope := range scopes {
		...
		if !ss(granted, scope) {
			return errorsx.WithStack(fosite.ErrInvalidScope.WithHintID(i18n.ErrHintScopeNotGranted, scope))
		}
	}
...
func (c *CoreValidator) introspectAccessToken(ctx context.Context, token string, accessRequest fosite.AccessRequester, scopes []string) error {
	...
	if err != nil {
		return errorsx.WithStack(fosite.ErrRequestUnauthorized.WithHintID(i18n.ErrHintInvalidCredentialFormat).WithWrap(err).WithDebug(err.Error()))
