 ...
 */
 ...
 func (d *DeviceAuthorizationHandler) HandleTokenEndpointRequest(ctx context.Context, requester fosite.AccessRequester) error {
	 ...
	 if !requester.GetClient().GetGrantTypes().Has(deviceCodeGrantType) {
		 return errorsx.WithStack(fosite.ErrUnauthorizedClient.WithHint("The OAuth 2.0 Client is not allowed to use device authorization grant \"" + deviceCodeGrantType + "\"."))
	 }
 
	 deviceCode := requester.GetRequestForm().Get("device_code")
	 signature := d.DeviceCodeStrategy.DeviceCodeSignature(deviceCode)
 
	 deviceAuthorizeRequest, err := d.DeviceCodeStorage.GetDeviceCodeSession(ctx, signature, requester.GetSession())
	 ...
	 // Override scopes
	 requester.SetRequestedScopes(deviceAuthorizeRequest.GetRequestedScopes())
 
	 // Override audiences
	 requester.SetRequestedAudience(deviceAuthorizeRequest.GetRequestedAudience())
 
	 // The authorization server MUST ensure that the device code was issued to the authenticated
	 // confidential client, or if the client is public, ensure that the
	 // code was issued to "client_id" in the request,
	 if deviceAuthorizeRequest.GetClient().GetID() != requester.GetClient().GetID() {
	 ...
	 // in Section 3.2.1.
	 requester.SetSession(deviceAuthorizeRequest.GetSession())
	 requester.SetID(deviceAuthorizeRequest.GetID())
