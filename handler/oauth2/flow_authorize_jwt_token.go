...
type AuthorizeJwtGrantHandler struct {
	...
	// JWTTokenMaxDuration sets the maximum time after token issued date, during which the token is considered valid.
	JWTTokenMaxDuration time.Duration
...
func (c *AuthorizeJwtGrantHandler) validateToken(ctx context.Context, token *jwt.JSONWebToken, key *jose.JSONWebKey) error {
	...
	if claims.IssuedAt != nil && time.Now().Sub(claims.IssuedAt.Time()).Nanoseconds() > c.JWTTokenMaxDuration.Nanoseconds() {
		return errorsx.WithStack(fosite.ErrInvalidGrant.
			WithHintf(
