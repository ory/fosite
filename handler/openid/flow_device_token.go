...
func (c *OpenIDConnectDeviceHandler) PopulateTokenEndpointResponse(ctx context.Context, requester fosite.AccessRequester, responder fosite.AccessResponder) error {
	...
	ar, err := c.OpenIDConnectRequestStorage.OpenIDConnectRequestStorage().GetOpenIDConnectSession(ctx, signature, requester)
	...
	err = c.OpenIDConnectRequestStorage.OpenIDConnectRequestStorage().DeleteOpenIDConnectSession(ctx, deviceCode)
	if err != nil {
		return errorsx.WithStack(fosite.ErrServerError.WithWrap(err).WithDebug(err.Error()))
	}
